...
import { getContrastRatio, checkContrast, formatRatio, getWCAGLevel, COMMON_COMBINATIONS } from "./contrast";
...
describe("COMMON_COMBINATIONS", () => {
  it("all light mode combinations pass AA large", () => {
    for (const [, combo] of Object.entries(COMMON_COMBINATIONS).filter(([k]) => k.startsWith("light"))) {
