...
/**
 * Netlify serverless function to generate image metadata using Claude Vision API
...
export const handler: Handler = async (
  ...
  try {
    ...
    const { imageUrl }: RequestBody = JSON.parse(event.body);
...
Return ONLY valid JSON with no additional text, in this exact format:
{
  ...
  "category": "..."
    ...
    return {
      ...
      headers: {
        "Content-Type": "application/json",
        // Allow requests from Studio (adjust as needed for CORS)
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Headers": "Content-Type",
        "Access-Control-Allow-Methods": "POST, OPTIONS",
      },
