...
export const siteSettingsQueryOptions = (preview = false) =>
  queryOptions({
    queryKey: [...queryKeys.siteSettings(), { preview }],
    queryFn: async (): Promise<GetSiteSettingsQueryResult | null> => {
      try {
        return await getSanityClient(preview).fetch(getSiteSettingsQuery);
      } catch (error) {
...
export function useSiteSettings(preview = false) {
  return useQuery(siteSettingsQueryOptions(preview));
}
