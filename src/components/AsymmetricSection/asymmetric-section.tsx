 ...
 * Creates more dynamic, organic flow than traditional symmetric grids
 */
...
export default function AsymmetricSection({
  ...
  const contentWidth = imageSize === "small" ? "lg:w-2/3" : imageSize === "medium" ? "lg:w-3/5" : "lg:w-1/2";
      ...
      {imagePosition === "left" ? (
        <>
          <div className={`${imageWidth}`}>
            <div className={curved ? "overflow-hidden rounded-3xl" : ""}>
              {imageSlot}
            </div>
          </div>
        ...
        <>
          <div className={`${contentWidth} order-2 lg:order-1`}>
            {contentSlot}
          </div>
